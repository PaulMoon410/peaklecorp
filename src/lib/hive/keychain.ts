...
import { HiveAccount, HiveTransaction, HiveKeychainRequestResponse } from '@/types/hive';

declare global {
  interface Window {
    hive_keychain?: {
      isInstalled?: boolean;
      requestSignBuffer: (username: string, message: string, method: string, callback: (response: HiveKeychainRequestResponse) => void) => void;
      requestBroadcast: (username: string, operations: any[], method: string, callback: (response: HiveKeychainRequestResponse) => void) => void;
      requestSignTx: (username: string, tx: any, method: string, callback: (response: HiveKeychainRequestResponse) => void) => void;
      requestTransfer: (username: string, to: string, amount: string, memo: string, currency: string, callback: (response: HiveKeychainRequestResponse) => void) => void;
      requestAddAccountAuthority: (username: string, authorizedUsername: string, role: string, weight: number, callback: (response: HiveKeychainRequestResponse) => void) => void;
    };
  }
}
...
export class HiveKeychainService {
  ...
  public isKeychainInstalled(): boolean {
    return typeof window !== 'undefined' && window.hive_keychain?.isInstalled === true;
  }
  ...
  public async initialize(): Promise<boolean> {
    try {
      ...
      // HiveKeychain doesn't require explicit connection like MetaMask
   ...
   */
  public async requestLogin(username: string): Promise<{ success: boolean; username?: string; error?: string }> {
      ...
      window.hive_keychain!.requestSignBuffer(
        ...
        (response: HiveKeychainRequestResponse) => {
          if (response.success) {
            ...
            resolve({ success: false, error: response.message || 'Login failed' });
          }
        }
      );
    });
      ...
      window.hive_keychain!.requestBroadcast(
        ...
        (response: HiveKeychainRequestResponse) => {
          if (response.success) {
            ...
            resolve({ success: false, error: response.message || 'Transaction failed' });
          }
        }
      );
    });
      ...
      window.hive_keychain!.requestTransfer(
        ...
        (response: HiveKeychainRequestResponse) => {
          if (response.success) {
            ...
            resolve({ success: false, error: response.message || 'Transfer failed' });
          }
        }
      );
    });
  ...
  public async processBatchTransactions(
    ...
    for (const tx of transactions) {
      try {
        const result = await this.broadcastTransaction(username, tx.operations, tx.method);
        if (result.success) {
